...
namespace Aryzon {
	...
	public class ARCameraShifter {
		...
		public float reticleDistance = 0.5f;

		public float sceneUnitsPerMeter = 1f;
		...
		void Start () {
			if (sceneUnits == Units.Centimeters) {
				sceneUnitsPerMeter = 100f;
			} else {
				sceneUnitsPerMeter = 1f;
			}

			Reticle reticle = right.GetComponent<Reticle> ();
			if (reticle) {
				reticle.m_DefaultDistance = reticleDistance;
			}
		...
		public void UpdateLayoutFromEvent () {
			Debug.Log ("[Aryzon] Updating layout from event");
			UpdateLayout ();
		}*/

		public void UpdateLayout () {
            //AryzonSettings.Headset.lensCenterDistance = 0.065f;

			if (!singleMode) {
				left.transform.localPosition = new Vector3 (-(sceneUnitsPerMeter * AryzonSettings.Calibration.IPD / 2), 0f, sceneUnitsPerMeter * -AryzonSettings.Headset.eyeToLens);
				right.transform.localPosition = new Vector3 ((sceneUnitsPerMeter * AryzonSettings.Calibration.IPD / 2), 0f, sceneUnitsPerMeter * -AryzonSettings.Headset.eyeToLens);
			} else {
				right.transform.localPosition = new Vector3 (0f, 0f, sceneUnitsPerMeter * -AryzonSettings.Headset.eyeToLens);
			}

			if (AryzonSettings.Calibration.rotatedSensor) {
				cameras.transform.localPosition = new Vector3 (-sceneUnitsPerMeter * AryzonSettings.Calibration.xShift, -sceneUnitsPerMeter * (AryzonSettings.Calibration.yShift + AryzonSettings.Headset.bottomToCenter), 0f);
			} else {
				cameras.transform.localPosition = new Vector3 (sceneUnitsPerMeter * AryzonSettings.Calibration.xShift, sceneUnitsPerMeter * (AryzonSettings.Calibration.yShift + AryzonSettings.Headset.bottomToCenter), 0f);
			}
		...
		void updateCam(Camera cam) {
			...
			if (singleMode) {
				...
				if (AryzonSettings.Calibration.rotatedSensor) {
					camTransform.localPosition = new Vector3 (multiplier * (sceneUnitsPerMeter * (AryzonSettings.Calibration.IPD) / 2), 0f, sceneUnitsPerMeter * -AryzonSettings.Headset.eyeToLens);
					camTransform.localEulerAngles = new Vector3 (0f, 0f, 180f);
				} else {
					camTransform.localPosition = new Vector3 (multiplier * (sceneUnitsPerMeter * (AryzonSettings.Calibration.IPD) / 2), 0f, sceneUnitsPerMeter * -AryzonSettings.Headset.eyeToLens);
				}
			}
		...
		float mWidth() {
			...
			if (AryzonSettings.Phone.aryzonCalibrated && !Application.isEditor) {
			...
			return (pixelWidth / pixelsPerInch) * 0.0254f;
		}
